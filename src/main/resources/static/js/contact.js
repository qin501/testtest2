		...
		for (var i = 0 ; i < friendList.length ; i ++) {
			...
			contactHtml += ' <li id="'+ friend.friendUserId +'" class="chatli_box" style="display : none"> ' +
								...
								' <div class="windows_body"> ' +
									' <div id="scroll_div_'+ friend.friendUserId +'"  style=" overflow:scroll;  height:420px;"> ' +
...
//							console.log("构建。。。。。。。。。。。。");
		...
		document.getElementById("talk_window_chat").innerHTML = contactHtml;
            for (var i = 0; i < friendList.length; i++) {
            	try {
                    UE.delEditor('input_box_' + friendList[i].friendUserId);
                }catch(e){
            		console.log("exeception:"+e);
				}
                //console.log('input_box_'+friendList[i].friendUserId);
                UE.getEditor('input_box_' + friendList[i].friendUserId, {
                    ...
                    wordCount: false,
                    toolbars: [[
                    'source','undo','redo','emotion','simpleupload','map','insertvideo'
                ]]
                });
            }
